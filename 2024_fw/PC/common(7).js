jQuery(document).ready(function(){
    ...
    upDownArrowEvent();
    newProductEvent();
    timeLimitEvent(3);   // 3개 노출시에 3, 5개노출시에 5 를 인자로 입력한다.         
    detailMoreBtnEvent();    
    windowEventRelease();
    hcNewArrivalsSlide();
    placeholderSet();
...
function mainBottomSlide() {
    if(jQuery('#hncokSwiperSlideMain').length < 1) return;
    const swiper = new Swiper('#hncokSwiperSlideMain', {
      loop: true,
      navigation: {
...
function hcWithDraw() {
     if (confirm('회원탈퇴를 하시겠습니까?')) location.href = '/shop/mypage.html?mypage_type=myexituser';
}
function placeholderSet() {
    jQuery('input#name').attr('placeholder','이름');
    jQuery('input#find_id_email').attr('placeholder','이메일');
    jQuery('input#find_id_mobile').attr('placeholder','휴대폰번호');
    jQuery('.hcIdWrap input').attr('placeholder','아이디');
    jQuery('.hcPwdWrap input').attr('placeholder','비밀번호');
    jQuery('.hcEmailWrap input').attr('placeholder','이메일');   
    jQuery('#find_pw_mobile_wrap input').attr('placeholder','휴대폰번호');   
    jQuery('.hcOrderName input').attr('placeholder','주문자명');   
    
    jQuery('.hcEtcPhoneWrap input').attr('placeholder','휴대폰번호');   
    jQuery('.hcAuthNumWrap input').attr('placeholder','인증번호');   
    

}
function hcNewArrivalsSlide() {
    if(jQuery('#hcNewArrivals').length < 1) return;
    var swiper = new Swiper("#hcNewArrivals", {
      slidesPerView: 3.5,
      spaceBetween: 12.5,
      freeMode: true,    
      scrollbar: {
...
function randowMainImageEvent() {
    ...
    setTimeout(function() {
        ...
        var loginColor = jQuery('#hcMainImage div.mainImage[data-order="'+showOrder+'"]').attr('login-color') || 'black';        
        ...
        jQuery('#hcLogin path').attr('stroke',loginColor);
