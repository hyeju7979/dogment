jQuery(document).ready(function() {    
    HeaderMenuOpen.init();
    //RandomImage.init();
    HcMainSlide.init('pigmentMainSlide');  //오픈시주석해제
    UpDownArrow.init();
    HcTimeLimit.init('hcTimeSaleWrap',true);
    //HcTimeLimit.init('hcTodaySaleWrap',false);
    HcMoreButton.init();
    oldsnapfit();    
    wearepigment();
    HcOpenRun.init();
...
function hcMiddleSlide() {

    if(jQuery(".hcMiddleSlide").length < 1) return;
    
    var listSwiper = new Swiper(".hcMiddleSlide", {
        scrollbar: {
            el: '.hcMiddleSlide .swiper-scrollbar',
            hide: false,
          },
...
function hcWithDraw() {
     if (confirm('회원탈퇴를 하시겠습니까?')) location.href = '/m/mypage.html?mypage_type=myexituser';
}
...
var HcOpenRun = (function(){

    var init = function() {
        if(jQuery('#mainRemainTime').length < 1) return;
        action.startTimer();
    }
    ...
    var action = {
        getTimeUntilNextReset : function() {
            ...
            nextReset.setHours(10, 0, 0, 0); // 오전 10시로 설정

            // 현재 시간이 오전 10시 이후라면, 다음 날 오전 10시로 설정
            if (now.getHours() >= 10) {
        ...
        startTimer : function() {
            function updateTimer() {
                const timeUntilNextReset = action.getTimeUntilNextReset();

                const hours = Math.floor((timeUntilNextReset / (1000 * 60 * 60)) % 24);
                const minutes = Math.floor((timeUntilNextReset / (1000 * 60)) % 60);
                const seconds = Math.floor((timeUntilNextReset / 1000) % 60);
                
                const pad = (n) => n < 10 ? '0' + n : n;
                
                jQuery('#mainRemainTime').text(pad(hours)+":"+pad(minutes)+":"+pad(seconds));

               // console.log(`남은 시간: ${hours}시간 ${minutes}분 ${seconds}초`);
            }
            updateTimer(); // 처음 실행 시 즉시 업데이트
...
//추가메인슬라이드
var HcMainSlide = (function(){
    var init = function(wrapId) {
        action.slickSlide(wrapId);
    }

    var action = {
        slickSlide : function(wrapId) {
            jQuery('#'+wrapId).slick({
                prevArrow: '<button type="button" class="slick-arrow hc-main-slick-prev">Previous</button>',           
                nextArrow:'<button type="button" class="slick-arrow hc-main-slick-next">Next</button>',
                slidesToShow: 1,
                dots:true,
                 arrows: false,   
             });
...
var HcFullPage = (function(){
    ...
    var handler = {
        isExist : function() {
            return action.isExist();
        },
        moveTo : function(sectionNo) {
            action.moveTo(sectionNo);
        },
...
var HcTimeLimit = (function() {
    var init = function(wrapId,isSlide) {

        if(!action.isExist(wrapId)) return;

        action.initTimeLimit(wrapId);
        if(isSlide) {
    ...
    var action = {
        ...
        initTimeLimit : function(wrapId) {
            var remainSec = action.getRemainSecTargetHour(10);

            action.setRemainSec(remainSec);
            jQuery('#'+wrapId).hcTimeLimit({
        ...
        setRemainSec : function(remainSec) {
            jQuery('li[data-remain-sec]').attr('data-remain-sec',remainSec);
        },
        getRemainSecTargetHour : function(targetHour) {
            // 현재 시간을 가져옵니다.
        ...
        isExist : function(wrapId) {
            return jQuery('#'+wrapId).length ? true : false;
        }
...
var AnimateAction = (function(){
    var action = {
        animatedEvent : function(destination,eventclass,event) {
    ...
    var handler = {
        action : function(destination,eventclass,event) {        
            action.animatedEvent(destination,eventclass,event);
        }    
    };
    ...
    var bind = function() {
        jQuery('div.hc_badge_up a').on('click',function(e){
            ...
            if(HcFullPage.handler.isExist()) {
                HcFullPage.handler.moveTo(1);
            }else{
                action.animateScroll(0);                
            }
        ...
        jQuery('div.hc_badge_down a').on('click',function(e){
            ...
            if(HcFullPage.handler.isExist()) {
                ...
                action.animateScroll(jQuery(document).height());                                 
            }
...
var RandomImage = (function(){
    ...
    var action = {
        randomImage : function() {  
            var $mainImage = jQuery('#hcMainImage li.mainImage');                 
            var showOrder = action.getRandomMainImageOrder(1,$mainImage.length);                         
            action.setImage(showOrder);
            action.setIconColor(showOrder);  
        },
        setImage : function(showOrder) {            
            jQuery('#hcMainImage .loadingImage').hide();
            //jQuery('#hcMainImage li.mainImage[data-order="'+showOrder+'"] img').addClass('open')
            //jQuery('#hcMainImage li.mainImage').hide();
            jQuery('#hcMainImage li.mainImage[data-order="'+showOrder+'"]').fadeIn();
        },
        setIconColor : function(showOrder) {
            var menuColor = jQuery('#hcMainImage li.mainImage[data-order="'+showOrder+'"]').attr('menu-color') || 'black';
            var logoColor = jQuery('#hcMainImage li.mainImage[data-order="'+showOrder+'"]').attr('logo-color') || 'black';
            var searchColor = jQuery('#hcMainImage li.mainImage[data-order="'+showOrder+'"]').attr('search-color') || 'black';
            var cartColor = jQuery('#hcMainImage li.mainImage[data-order="'+showOrder+'"]').attr('cart-color') || 'black';
            var myPageColor = jQuery('#hcMainImage li.mainImage[data-order="'+showOrder+'"]').attr('mypage-color') || 'black';

            jQuery('#hc_hamburger span').css('background-color',menuColor);
            jQuery('#mainLogo a').css('color',logoColor);
            jQuery('#hcHeadSearch path').attr('stroke',searchColor);
            jQuery('#hcHeadMypage path').attr('stroke',myPageColor);
            jQuery('#hc_head_cart path').attr('stroke',cartColor);
            
        },
        getRandomMainImageOrder : function(min, max) {
...
var HeaderMenuOpen = (function(){
	...
	var bind = function() {
        ...
        jQuery('#hc_hamburger').on('click',function(){
         ...
         jQuery('#hcHeaderCloseBtn').on('click',function(){                
                action.toggle(data[1]);
                action.setIsOpen(1,false);                
            });
	...
	var action = {
        ...
        zIndexCal(closeLayerIndex,openLayerIndex) {
            ...
            jQuery('#'+data[openLayerIndex]['layerId']).css('z-index',parseInt(closeLayerZindex)+1);
